...
def train(env_name,train_steps = 100000,suffix="",Ktrain_samples=20000):   
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "DATA/LSPN_Hyperparameter_data/"+suffix+"/KNonlinearmamba_"+env_name+"samples{}".format(Ktrain_samples)
    if not os.path.exists( "DATA/LSPN_Hyperparameter_data/"+suffix):
        os.makedirs( "DATA/LSPN_Hyperparameter_data/"+suffix)
    ...
    print(f"{suffix} Total parameters: {total_params}")
...
if __name__ == "__main__":
    ...
    for i in env_names:
        ...
        parser.add_argument("--suffix",type=str,default="mamba_test_kstep50_kbatchsize100_d_state8_d_conv4_expand4")#Ksteps,Ktest_data,Ktrain_data
